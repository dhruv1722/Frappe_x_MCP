...
class TransportSettingsTests(unittest.TestCase):
	...
	def test_missing_transport_defaults_to_stdio(self):
		...
		self.assertEqual(settings.module, "sales")

	def test_unsupported_module_fails_before_startup(self):
		with self.assertRaisesRegex(RuntimeError, "MCP_MODULE"):
			_settings(module="accounts").validate_module()
	...
	def test_http_allows_a_configured_service_identity(self):
		_settings(identity_mode="service", frappe_user="mcp-service@example.com").validate_transport()

	def test_http_service_identity_requires_a_frappe_user(self):
		with self.assertRaisesRegex(RuntimeError, "MCP_FRAPPE_USER"):
			_settings(identity_mode="service", frappe_user=None).validate_transport()
