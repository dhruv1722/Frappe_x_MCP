...
class QuotationServiceTests(unittest.TestCase):
	def setUp(self):
		...
		self.patches = [
			...
			patch.object(quotation_service, "resolve_customer", side_effect=self._resolve_customer),
			patch.object(quotation_service, "resolve_sales_item", side_effect=self._resolve_sales_item),
		]
	...
	def _resolve_customer(self, query):
		if query in self.available_customers:
			return {"status": "resolved", "candidate": {"value": query}, "match_type": "exact"}
		return {"status": "not_found", "candidates": []}

	def _resolve_sales_item(self, query):
		if query in self.available_items:
			return {"status": "resolved", "candidate": {"value": query, "item_name": query}, "match_type": "exact"}
		return {"status": "not_found", "candidates": []}
	...
	def test_valid_existing_customer_and_item_prepare_without_insert(self):
		...
		self.assertEqual(result["preview"]["customer"], "CUST-001")
	...
	def test_invalid_customer_or_item_is_safely_rejected(self):
		customer = self.prepare(customer="MISSING")
		item = self.prepare(items=[self.item("MISSING")])
		self.assertEqual(customer["status"], "not_found")
