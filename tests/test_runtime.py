...
class RuntimeIdentityTests(unittest.TestCase):
	@patch("mcp_erpnext.runtime._run_http_tool", return_value={"ok": True})
	@patch("mcp_erpnext.runtime.MCPSettings.from_environment")
	def test_http_service_mode_never_reads_a_request_identity(self, from_environment, run_http_tool):
		from_environment.return_value = MCPSettings(
			**{
				**_librechat_settings().__dict__,
				"identity_mode": "service",
				"frappe_user": "mcp-service@example.com",
				"transport": "streamable-http",
				"http_shared_secret": "this-is-a-development-only-secret-with-32-chars",
			}
		)

		self.assertEqual(runtime.execute_tool_with_context(Mock(), "search_customers", lambda: {}), {"ok": True})
		self.assertIsNone(run_http_tool.call_args.args[1])
