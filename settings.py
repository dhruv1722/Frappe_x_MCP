...
class MCPSettings:
	...
	def from_environment(cls) -> "MCPSettings":
		return cls(
			...
			module=os.environ.get("MCP_MODULE", "sales").strip().lower(),
		)
	...
	def validate_module(self) -> None:
		"""Reject a module before its MCP process starts."""
		if self.module not in SUPPORTED_MODULES:
			supported = ", ".join(SUPPORTED_MODULES)
			raise RuntimeError(f"MCP_MODULE must be one of: {supported}.")
	...
	def validate_transport(self) -> None:
		...
		if self.identity_mode == "service" and not (self.frappe_user or "").strip():
			raise RuntimeError("MCP_FRAPPE_USER is required for Streamable HTTP service identity.")
