...
def _prepare_items(items: Any) -> dict[str, Any]:
	...
	for index, raw_item in enumerate(items, start=1):
		...
		rate = _coerce_rate(raw_item.get("rate", raw_item.get("price")))
		if raw_item.get("qty", raw_item.get("quantity")) not in (None, "") and quantity is None:
			return _error("INVALID_QUOTATION_DETAILS", f"Item quantity on row {index} must be a positive number.")
		if raw_item.get("rate", raw_item.get("price")) not in (None, "") and rate is None:
			return _error("INVALID_QUOTATION_DETAILS", f"Item rate on row {index} must be a non-negative number.")
		if quantity is None:
...
def prepare_quotation(
	...
	customer_name = customer_resolution["candidate"]["value"]
	...
	doc.party_name = customer_resolution["candidate"]["value"]
	...
	for row, item in zip(doc.items, item_result["items"]):
		if item["rate"] is not None:
			row.rate = item["rate"]
			row.price_list_rate = item["rate"]
