...
def _coerce_rate(value: Any) -> float | None:
	"""Accept an optional explicit sales rate without allowing invalid values."""
	if value in (None, ""):
		return None
	if isinstance(value, bool):
		frappe.throw("Item rate must be a non-negative number.", frappe.ValidationError)
	try:
		rate = float(value)
	except (TypeError, ValueError) as exc:
		raise frappe.ValidationError("Item rate must be a non-negative number.") from exc
