...
def execute_tool_with_context(
	...
	"""Execute one MCP tool with persistent STDIO or scoped HTTP runtime state."""
	...
	# HTTP service mode always uses the configured MCP_FRAPPE_USER. Only the
	# LibreChat identity mode reads a per-request identity header.
	runtime_identity = (
		get_http_runtime_identity(context) if settings.identity_mode == "librechat" else None
	)
	return execute_tool(tool_name, lambda: _run_http_tool(settings, runtime_identity, operation))
...
def _run_http_tool(
	settings: MCPSettings, runtime_identity: RuntimeIdentity | None, operation: Callable[[], ResultT]
) -> ResultT:
	if settings.transport != "streamable-http":
